...
public abstract class Classifier {
	...
	for (Option<?> option : options()) {
	    if (option.key != TEST_FILE && option.key != TRAINING_FILE) {
		String modifier = null;

		if (option.hasArgument()) {
		    modifier = String.format("-%s",
     ...
     *
     */
    public void constructModel(String toolName, String[] args) throws Exception {
    	// sets property defaults
    	defaults();
    	// reads command-line arguments
    	Map<String, String> parameters = processCommandLine(args);
    	...
    	if (CONFIG.isPresent(TRAINING_FILE)) {
    	    ...
    	    throw new IllegalArgumentException("Inappropriate input arguments for model geenration");
    	}
     ...
     *
     */
    public void classifyData(String[] args, Model model) throws Exception {
    	// sets property defaults
    	defaults();
    	// reads command-line arguments
    	Map<String, String> parameters = processCommandLine(args);        	
    	if (CONFIG.isPresent(TEST_FILE)) {
    		...
    		ARFFReader reader = new ARFFReader();
    		Dataset dataset = reader.read(CONFIG.get(TEST_FILE));
    		
    		logRuntime(dataset, parameters);

    	    long start = System.nanoTime();

    		Logger.log("%n=== Classifying test data set ===%n%n");
    		//todo
    		for (int i = 0; i < dataset.size(); i++)
            {                
                int predicted = model.classify(dataset, i);

                dataset.setValue(i,dataset.classIndex(),predicted);
            }
    		
    		double elapsed = (System.nanoTime() - start) / Math.pow(10, 9);
    		
    		dataset.getArffFormattedList();
    		    		
    	    Logger.log("%nRunning time for classification (seconds): %.2f%n", elapsed);
    	    Logger.close();    	    
    	    
    	} else {
    		throw new IllegalArgumentException("Inappropriate input arguments for classification");
    	}    	    
