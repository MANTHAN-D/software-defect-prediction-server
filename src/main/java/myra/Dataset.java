...
public final class Dataset {
     ...
     * @author Manthan
     * 
     * Sets the attribute value of a given instance.
     * 
     * @param instance
     *            the instance index.
     * @param attribute
     ...
     * 			the attribute value.
     */
    public void setValue(int instance, int attribute, int value) {
    	
    	instances[(instance * attributes.length) + attribute] = value;    	
    }
     ...
     * @author Manthan
     * 
     * Returns the list of classified data in ArffModel Object format.
     * 
     * @return list of ArffModel objects
     */
    public void getArffFormattedList() {
    	
    	List<ArffModel> resultList = new ArrayList<>();
    	ArffModel arffModel;
    	
    	for (int instance = 0; instance < this.size(); instance++){
    		arffModel = new ArffModel();    		
    		
    		arffModel.setWmc(instances[(instance * attributes.length) + 0]);
    		arffModel.setDit(instances[(instance * attributes.length) + 1]);
    		arffModel.setNoc(instances[(instance * attributes.length) + 2]);
    		arffModel.setCbo(instances[(instance * attributes.length) + 3]);
    		arffModel.setRfc(instances[(instance * attributes.length) + 4]);
    		arffModel.setLcom(instances[(instance * attributes.length) + 5]);
    		int predictedClassIndex = (int)instances[(instance * attributes.length) + 6];
    		arffModel.setBugFreq( attributes[this.classIndex()].value(predictedClassIndex)); 		
    		
    		resultList.add(arffModel);
    	}    	    	
    	ModelHandler.setResponseDataList(resultList);
    }
}